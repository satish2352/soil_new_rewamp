...
import { submitJobApplication, ValidationError } from '../../lib/api';
 ...
 * Mirrors the legacy `employment_form`: name, email, mobile, qualification,
 * experience_from, experience_to, address, resume → /api/frontjobpostingadd
 */
export default function JobVacancyForm() {
  ...
  async function handleSubmit(e) {
    ...
    // A range that runs backwards is always a typo; catch it before the API does.
    if (
      values.experience_from &&
      values.experience_to &&
      values.experience_to < values.experience_from
    ) {
      found.experience_to = `${t('form.to')} ≥ ${t('form.from')}`;
    }
    ...
    setSending(true);
    try {
      const res = await submitJobApplication({ ...values, resume });
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <fieldset className="rounded-2xl border border-line bg-cream/40 p-5">
        <legend className="px-2 text-fluid-sm font-semibold text-deep">
          {t('form.experience')}
        </legend>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field
            ...
            onChange={set('experience_from')}
            error={errors.experience_from}
          />
          <Field
            label={t('form.to')}
            name="experience_to"
            ...
            onChange={set('experience_to')}
            error={errors.experience_to}
          />
        </div>
      </fieldset>
